...
interface TeamRow { slug: string; name: string; flagCode: string; }
...
@Component({
  ...
  template: `
    <header class="page-header" style="padding: 0 0 var(--space-md); display: flex; justify-content: space-between; align-items: baseline; flex-wrap: wrap; gap: var(--space-md);">
      <div>
        <small>{{ teams().length }} equipos cargados · 48 esperados</small>
    ...
    @if (loading()) {
      ...
      <div class="standings-wrap">
        <table class="standings standings--group">
          <thead>
            <tr><th>Bandera</th><th>Slug</th><th>Nombre</th><th>Flag code</th><th>Acciones</th></tr>
          </thead>
          <tbody>
